...
def update_contents(docs_db_path, squad_path, num_workers=None):
    logger.info('Reading docs database: %s'%(docs_db_path))
    ...
    with open(squad_path) as f:
        ...
        for document in data:
            ...
            #print(utils.normalize(title))

            if title == 'Sky (United Kingdom)':
                title = 'Sky UK'

            c.execute("SELECT * FROM documents WHERE id = ?",(title,))
            data=c.fetchall()

            if len(data) ==0:
                print('not founded id(title): %s'%(title))
                assert false
            ...
            c.execute("UPDATE documents SET text = ? WHERE id = ?", (text, title))



    conn.commit()

    #c.execute("SELECT * FROM documents WHERE id = ?",('Super Bowl 50',))
    #for rows in c:
    #    print(rows)
...
if __name__ == '__main__':
    ...
    parser.add_argument('docs_db_path', type=str, help='/path/to/data')
    parser.add_argument('squad_path', type=str, help='/path/to/saved/db.db')
    ...
    update_contents(args.docs_db_path,args.squad_path,args.num_workers)
